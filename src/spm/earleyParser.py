...
#     print 'scan:',token.__class__.__name__,col.token.__class__.__name__
    if not token.__eq__(col.token):
        return
...
def parse(rule, lSeq):
    table = [Column(i, tok) for i, tok in enumerate([None] + lSeq)]
...
#                     print '**',table[i+1].token,table[i+1].token.__class__.__name__,term , term.__class__.__name__
