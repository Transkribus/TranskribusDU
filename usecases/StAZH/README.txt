...
#Training!!!
  > ./python.sh $srcDU/tasks/DU_StAZH.py trn3820 mdl-StAZH --trn trnskrbs_3820  --ecn
...
#TESTING!!!!!!!!!!!!!!!
  > ./python.sh $srcDU/tasks/DU_StAZH.py trn3820 mdl-StAZH  --tst trnskrbs_3832 --ecn
--------------------------------------------------
Trained model 'mdl-StAZH' in folder 'trn3820'
Test  collection(s):['C:\\tmp_READ\\tuto\\trnskrbs_3832\\col']
--------------------------------------------------
- loading a crf.Model_SSVM_AD3.Model_SSVM_AD3 model
        - loading pre-computed data from: trn3820\mdl-StAZH_model.pkl
                 file found on disk: trn3820\mdl-StAZH_model.pkl
                 file is fresh
        - loading pre-computed data from: trn3820\mdl-StAZH_transf.pkl
                 file found on disk: trn3820\mdl-StAZH_transf.pkl
                 file is fresh
 done
...
---> - Done, see in .\trnskrbs_3829
  ...
  > ./python.sh $srcDU/tasks/DU_StAZH.py trn3820 mdl-StAZH  --run trnskrbs_3829 --ecn
--> - done
...
#Actually, this collection was also annotated, so we can compute a score on it
  > ./python.sh $srcDU/tasks/DU_StAZH.py trn3820 mdl-StAZH  --tst trnskrbs_3829 --ecn
...
(unweighted) Accuracy score = 0.88
  ...
  ./python.sh $srcTR/TranskribusCommands/Transkribus_downloader.py 3820 --noimage
  ./python.sh $srcDU/tasks/DU_StAZH.py ./mdl-StAZH MyModel --trn trnskrbs_3820 --ecn


  ./python.sh $srcTR/TranskribusCommands/do_createCollec.py READDU_JL_TST
  ./python.sh $srcTR/TranskribusCommands/do_addDocToCollec.py 3571 3820 8251
  ./python.sh $srcTR/TranskribusCommands/Transkribus_downloader.py 3832
  ./python.sh $srcDU/tasks/DU_StAZH.py ./mdl-StAZH MyModel --tst trnskrbs_3832 --ecn 

  ./python.sh $srcTR/TranskribusCommands/do_createCollec.py READDU_JL_PRD
  ./python.sh $srcTR/TranskribusCommands/do_copyDocToCollec.py 3571 3829 8251 8252 8564-8566
  ./python.sh $srcTR/TranskribusCommands/Transkribus_downloader.py 3829 --noimage
  ./python.sh $srcDU/tasks/DU_StAZH.py ./mdl-StAZH MyModel --run trnskrbs_3829 --ecn
  ./python.sh $srcTR/TranskribusCommands/TranskribusDU_transcriptUploader.py ./trnskrbs_3829 3829
