...
from FeatureDefinition_PageXml_std      import FeatureDefinition_PageXml_StandardOnes_SEP
...
if __name__ == "__main__":
    ...
    parser.add_option("--BB31", dest='bBB31'    , action="store_true"
                      , help="New style BB (same width as baseline, resize of y)")     
    (options, args) = parser.parse_args()
    
    if not options.bSeparator:
        traceln("Forcing --sep option")
        options.bSeparator = True
        
    cFeatureDefinition = FeatureDefinition_PageXml_StandardOnes_SEP
    dFeatureConfig = { 'n_tfidf_node':500, 't_ngrams_node':(1,3), 'b_tfidf_node_lc':False
                        ,'bMultiPage':False
                        ,'bMirrorPage':False
                        }               
