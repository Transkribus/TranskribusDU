...
class My_GraphHierarchyConjugateSegmenter_MultiSinglePageXml_Separator(GraphHierarchyConjugateSegmenter_MultiSinglePageXml_Separator):
    ...
    def isEmpty(self):
        traceln("\tHACK to create a @row and @cell on nodes")
        for nd in self.lNode:
            ndParent = nd.node.getparent()
            _row  = "%s__%s" % ( ndParent.getparent().get("id")  # TABLE ID !
                              , ndParent.get("row"))
            _cell = "%s__%s" % (_row, ndParent.get("col"))
            nd.node.set("row" , _row)
...
# ----------------------------------------------------------------------------
class My_ConjugateNodeType_Row_Cell(NodeType_PageXml_type):
    """
    ...
    def __init__(self, sNodeTypeName, lsLabel, lsIgnoredLabel=None, bOther=True, BBoxDeltaFun=defaultBBoxDeltaFun, bPreserveWidth=False):
        super(My_ConjugateNodeType_Row_Cell, self).__init__(sNodeTypeName, lsLabel, lsIgnoredLabel, bOther, BBoxDeltaFun=BBoxDeltaFun, bPreserveWidth=bPreserveWidth)
...
def getConfiguredGraphClass(_doer):
    ...
    lsLabelAttribute=["row", "cell"]
    cNodeType=My_ConjugateNodeType_Row_Cell
