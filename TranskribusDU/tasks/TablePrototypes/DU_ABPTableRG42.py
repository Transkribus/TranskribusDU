...
from crf.Transformer_PageXml import NodeTransformerXYWH_v2, NodeTransformerNeighbors, Node1HotFeatures_noText
from crf.Transformer_PageXml import Edge1HotFeatures_noText, EdgeBooleanFeatures_v2, EdgeNumericalSelector_noText
...
class GraphGrid_H(GraphGrid):
    ...
    # the height of a block is proportionally reduced
    # we compute a deltaY, which is added to y1 and subtracted from y2
    # for example, to keep 3/5 of the height: BBoxDeltaFun = lambda v: v / 5.0
    BBoxDeltaFun = None     # to force explicit definition of it
    ...
    def showClassParam(cls):
        try:
            ...
            traceln("  - iGridStep_H : ", cls.iGridStep_H)
            traceln("  - iGridStep_V : ", cls.iGridStep_V)
            traceln("  - iGridVisibility  : ", cls.iGridVisibility)
            traceln("  - iBlockVisibility : ", cls.iBlockVisibility)
    ...
    @classmethod
    def reduceBlockHeight(cls, lNode, bRestore=False):
        """
        Reduce the height of block (typically before computing cuts)
        """
        if bRestore:
            for blk in lNode:
                #preserve (even if non-invertible function applied)
                blk.y1, blk.y2 = blk._y1_y2_
                del blk._y1_y2_
        else:
            for blk in lNode:
                #preserve (even if non-invertible function applied)
                blk._y1_y2_ = (blk.y1, blk.y2)
    
                _w,h = blk.getWidthHeight()
                dy = int(round(cls.BBoxDeltaFun(h)))
                # positive dy decreases block height
                y1 = blk.y1 + dy
                y2 = blk.y2 - dy
                
                #make sure we have at least height==1
                if y1 >= y2:
                    yg = (y1 + y2) / 2.0
                    y1 = int(round(yg))
                    y2 = y1 + 1
    ...
    def computeSpecialEdges(cls, lClassicPageNode, lSpecialPageNode):
        ...
        #first reduce the height of all classic nodes
        cls.reduceBlockHeight(lClassicPageNode)
        ...
        #then restore the height of all classic nodes
        cls.reduceBlockHeight(lClassicPageNode, bRestore=True)
...
class DU_ABPTableRG4(DU_CRF_Task):
    ...
    def getConfiguredGraphClass(cls):
        ...
        traceln(" - grid computation: BBoxDeltaFun = lambda v: v / %f" % cls.fDyRatio)
    ...
    def __init__(self, sModelName, sModelDir, 
        ...
        DU_ABPTableRG4.fDyRatio = fDyRatio
...
#     parser.add_option("--annotate", dest='bAnnotate',  action="store_true",default=False,  help="Annotate the textlines with BIES labels")    
    ...
    parser.add_option("--dy_ratio", dest='fDyRatio',  action="store", type=float,
                      default=DU_ABPTableRG4.fDyRatio,
                      help="Ratio used to reduce block height (H = H - 2 * H / r)") 
